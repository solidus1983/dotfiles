...
# preflight-ubuntu.sh and post-ubuntu.sh call these but don't define them
# themselves -- the real installer provides them before sourcing either.
...
# Bootstrap prerequisites (below) run before preflight-ubuntu.sh -- which
# defines LOG_FILE/run_quiet/gum's spinner -- is sourced, so a minimal
# LOG_FILE is needed here just for that one step. preflight-ubuntu.sh
# truncates and takes over the same path as soon as it runs.
